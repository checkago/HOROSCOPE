...
def _build_reference_relationship_block(relationship: Relationship) -> str:
    ...
    if source.name == "Лев" and target.name == "Куспид Водолей-Рыбы":
        lines.extend(
            [
                "",
                "**Эталон для этой пары:** протонный канал Льва и нейтронная суперпозиция куспида допускают дейтронный сценарий "
                "только в ветви, где нейтрон ведёт себя как связанный компонент. Поэтому в эталоне здесь корректно ставить "
                "`сильное` как ведущий режим, а не `слабое`.",
            ]
        )
