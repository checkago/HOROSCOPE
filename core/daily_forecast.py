...
def _axis_phrase(axis: str, score: int, seed: str) -> str:
    ...
    idx = int(hashlib.sha256(seed.encode("utf-8")).hexdigest(), 16) % len(seq)
    return seq[idx]
...
def build_daily_markdown(profile: Profile, forecast_date: date) -> str:
    ...
    energy_detail = _axis_phrase("energy", resilience_score, f"{key}:energy_detail:{profile.code}")
    focus_detail = _axis_phrase("focus", focus_score, f"{key}:focus_detail:{profile.code}")
    social_detail = _axis_phrase("social", social_score, f"{key}:social_detail:{profile.code}")
    risk_detail = _axis_phrase("risk", overload_risk, f"{key}:risk_detail:{profile.code}")
