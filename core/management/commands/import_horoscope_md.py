...
def parse_target(target_header: str) -> Tuple[str, str]:
    # Пример: "Женщиной-Куспид Водолей-Рыбы" / "Мужчиной-Лев"
...
def build_enriched_text(source: Profile, target: Profile, rel: Dict[str, str]) -> str:
    ...
    line3 = (
        ...
        "фиксирует наблюдаемую динамику системы."
    )
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS(f"Импорт завершен: {Profile.objects.count()} профилей, {Relationship.objects.count()} связей."))
